function dusaExport(name, cb)
    AddEventHandler(('__cfx_export_dusa_dispatch_%s'):format(name), function(setCB)
        setCB(cb)
    end)
end

---@param data table -- dusa_dispatch style alert data
---@return AlertData
local function ConvertDusaToKartikMDTAlert(data)
    ...
    return {
        ...
        description = data.description or "No description available",  -- Default value
        sound = data.alert and data.alert.sound or "dispatch",
        type = "Alert",
        person = (data.name and {
            name = data.name,
            phone = data.phone or "",  -- Ensure phone is not nil
        ...
        jobs = (function()
            local jobs = {}
            local rawJobs = data.recipientJobs or {'police'}
...
dusaExport('CustomDispatch', function(data)
    if not data then
        print("Error: No data received for CustomAlert.")
        return
    end
    local convertedData = ConvertDusaToKartikMDTAlert(data)
    exports['kartik-mdt']:CustomAlert(convertedData)
end)

RegisterNetEvent("dusa_dispatch:sendCustomDispatch",function(data)
    if not data then
        print("Error: No data received for CustomAlert.")
        return
    end
